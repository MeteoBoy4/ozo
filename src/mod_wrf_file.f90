...
contains
  ...
  function create_out_file ( fname, wrf_infile, mode, calc_b, forc, calc_div ) result ( f )
    character ( * ),   intent ( in ) :: fname ! file name
    type ( wrf_file ), intent ( in ) :: wrf_infile ! wrf inputfile
    logical,           intent ( in ) :: calc_b, forc, calc_div
    type ( wrf_file ) :: f
    ...
    allocate(f % pressure_levels(wrf_infile % nlev(1)))
    ...
    ! Create psfc variable
       ...
       if (calc_div) then
            ! Create khi variable
          status = nf90_def_var ( f % ncid, trim ( 'khi' ), NF90_FLOAT, &
               dimids, varid )
          if ( .not. ( status == nf90_enameinuse .or. status == NF90_NOERR ) ) &
               call check ( status )
          call check( nf90_put_att(f % ncid, varid,trim('description'),&
               trim('velocity potential') ) )
          call check( nf90_put_att(f % ncid, varid,trim('units'),&
               trim('m^2/s') ) )
        end if
...
!       dimids ( i ) = dimid
  ...
  function real2d ( file, time, names )
    ...
    allocate(real2d(file % dims(1), file % dims(2)))
    real2d = data ( names ( 1 ) )
    do i = 2, size ( names )
...
!    print*,'Reading ',trim(names(1))
    allocate(real3d(file % dims(1), file % dims(2), file % dims(3)))
    real3d = data ( names ( 1 ) )
